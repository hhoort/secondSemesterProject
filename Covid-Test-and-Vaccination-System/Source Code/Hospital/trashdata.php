...
INNER JOIN `hospital` AS h ON a.`hospital name` = h.`id` INNER JOIN `vaccines` AS v ON a.`vaccine` = v.`id` INNER JOIN `paitent` AS pp ON a.`pname` = pp.`id` where a.`Status2` = '0';
";
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Trash Data</h1>
    </div>
                            ...
                            if (mysqli_num_rows($pro_result) > 0) {
                                while ($pro_data = mysqli_fetch_assoc($pro_result)) {
                                    ...
                                    <tr>
                                        ...
                                        <td style="display: flex; justify-content:between;"><a
                                                href="restore.php?pid=<?php echo $pro_data['pid']; ?>"
                                                class="btn btn-success">Restore</a>
                                            <a href="delete.php?pid=<?php echo $pro_data['pid']; ?>"
