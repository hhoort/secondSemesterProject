...
INNER JOIN `hospital` AS h ON a.`hospital name` = h.`id` INNER JOIN `vaccines` AS v ON a.`vaccine` = v.`id` INNER JOIN `paitent` AS pp ON a.`pname` = pp.`id` where a.`PStatus` = 'Approve' and a.`Status2` = '1' and a.`PStatus33` = 'Sent To Lab' AND a.`Dose` = 'First';
";
                                ...
                                while ($pro_datas = mysqli_fetch_assoc($pro_results)) {
                                    ...
                                    <tr>
                                        ...
                                        <td style="display: flex; justify-content:space-around;"><a
                                                href="detail7.php?pid=<?php echo $pro_datas['pid']; ?>"
