...
INNER JOIN `hospital` AS h ON a.`hospital name` = h.`id` INNER JOIN `vaccines` AS v ON a.`vaccine` = v.`id` INNER JOIN `paitent` AS pp ON a.`pname` = pp.`id` where a.`PStatus` = 'Approve' and a.`Status2` = '1' and a.`PStatus3` = 'On The Way For Sample Collection';
";
                                ...
                                while ($pro_data = mysqli_fetch_assoc($pro_result)) {
                                    ...
                                    <tr>
                                        ...
                                        <td style="display: flex; justify-content:space-around;"><a
                                                href="onthewayupdate.php?pid=<?php echo $pro_data['pid']; ?>"
