...
$query = "SELECT * FROM `hospital` WHERE `Status` = 'Reject' LIMIT $start_from, $records_per_page ";
    ...
    <div class="content">
        <div class="row">
            <div class="col-sm-4 col-3">
                <h4 class="page-title">Reject Hospitals</h4>
            </div>
        </div>
                    ...
                    <div class="profile-widget">
                        ...
                        <div>
                            <?php if ($row['Status'] == 'Reject') {
                                echo "<td><span class='custom-badge status-red col-lg-12'>Reject</span></td>";
                            }
                            ?>
                        </div>
                    </div>
        ...
        $total_records = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM `hospital` WHERE `Status` = 'Reject'"));
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            if ($i == $current_page) {
                ...
                echo "<button class='btn mx-2'><a href='hospitalsreject.php?page=$i'>$i</a></button>";
            }
        }
        echo "</div>";
